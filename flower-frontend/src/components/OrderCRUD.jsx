...
const OrderCRUD = () => {
  ...
  const normalizeOrder = (order) => {
    const klienti = order.klienti || order.customer || null;

    return {
      ...order,
      klienti,
      clientName: klienti
        ? `${klienti.emri || klienti.firstName || ""} ${klienti.mbiemri || klienti.lastName || ""}`.trim()
        : "Guest",
      clientPhone: klienti?.telefoni || klienti?.phone || "—",
    };
  };
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      if (status === 403) {
        ...
        if (local.length > 0) {
          setOrders(local.map(normalizeOrder));
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    setOrders(prev =>
      prev.map(o => o.id === orderId ? { ...o, statusi: newStatus } : o)
    );
    ...
    try {
      ...
      localStorage.setItem(
        "orders",
        JSON.stringify(local.map(o => o.id === orderId ? { ...o, statusi: newStatus } : o))
      );
    } catch {
  ...
  const S = {
    ...
    wrap: { maxWidth: "1250px", margin: "0 auto" },
        ...
        {loading ? (
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={S.table}>
              <thead>
                <tr>
                  {["ID", "Klienti", "Numri", "Data", "Adresa", "Vlera (€)", "Statusi", "Veprime"].map(h => (
              ...
              <tbody>
                {orders.length === 0 ? (
                  <tr>
                    <td colSpan={8} style={S.empty}>Nuk ka porosi aktive.</td>
                  </tr>
                ) : (
                  orders.map((order, i) => {
                    ...
                    return (
                      <tr key={order.id} style={{ background: i % 2 === 0 ? "#FFF" : "#FDFAF7" }}>
                        <td style={{ ...S.td, fontWeight: "700", color: "#0E5A5B" }}>
                          #{order.id}
                        </td>
